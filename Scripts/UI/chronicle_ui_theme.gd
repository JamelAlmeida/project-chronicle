...
const COLOR_HEALTH := Color(0.78, 0.16, 0.16, 1.0)
const COLOR_STEADFAST := Color(0.20, 0.48, 0.82, 1.0)
const COLOR_XP := Color(0.24, 0.52, 0.86, 1.0)
const COLOR_TECHNIQUE := Color(0.52, 0.36, 0.74, 1.0)
const COLOR_QUEST := Color(0.72, 0.82, 0.48, 1.0)
...
static func panel_style(background: Color = COLOR_PANEL, border: Color = COLOR_BRASS) -> StyleBox:
	var textured := textured_style("panel_compact_9.png", 28.0, 14.0)
	if textured == null:
		textured = textured_style("panel_header_9.png", 24.0, 12.0)
	if textured != null:
		return textured
...
static func hud_panel_style(background: Color = COLOR_PANEL, border: Color = COLOR_BRASS) -> StyleBox:
	var textured := textured_style("panel_tracker_9.png", 18.0, 10.0)
	if textured == null:
		textured = textured_style("panel_header_9.png", 18.0, 8.0)
	if textured != null:
		...
		return tex_style
	return _flat_panel(background, border.lightened(0.08), 1, 10.0, 8.0, 3)
...
static func _island_style_named(
	...
	if textured != null:
		...
		return tex_style
	return _flat_panel(
		Color(0.045, 0.035, 0.025, 0.92),
		Color(0.55, 0.42, 0.22, 0.80),
		1,
		12.0,
		8.0,
		8
...
static func inset_style(accent: Color = Color(0.22, 0.18, 0.14, 1.0)) -> StyleBox:
	var textured := textured_style("panel_section.png", 18.0, 8.0)
	if textured == null:
		textured = textured_style("panel_compact_9.png", 18.0, 8.0)
	if textured != null:
		return textured
...
static func action_slot_style(accent: Color, emphasized: bool = false) -> StyleBox:
	...
	if emphasized:
		...
		flat.border_color = Color(0.52, 0.40, 0.24, 0.75).lerp(accent, 0.12)
...
static func equipment_slot_style() -> StyleBox:
	var tex := runtime_texture("slot_empty_9.png")
	if tex == null:
		tex = runtime_texture("equip_slot.png")
	if tex != null:
		var style := StyleBoxTexture.new()
		style.texture = tex
...
static func section_style(accent: Color = COLOR_BRASS) -> StyleBox:
	var textured := textured_style("panel_section.png", 20.0, 10.0)
	if textured == null:
		textured = textured_style("panel_compact_9.png", 20.0, 10.0)
	if textured != null:
		return textured
...
static func bar_background() -> StyleBox:
	var textured := textured_style("bar_empty_9.png", 6.0, 1.0)
	if textured == null:
		textured = textured_style("bar_thin_9.png", 5.0, 1.0)
	if textured != null:
		return textured
...
static func style_button(button: Button, accent: Color = COLOR_BRASS) -> Button:
	...
	var normal := textured_style("btn_chrome_9.png", 14.0, 8.0)
	if normal == null:
		normal = textured_style("btn_menu.png", 14.0, 8.0)
	if normal == null:
